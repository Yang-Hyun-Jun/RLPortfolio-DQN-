...
"""
state: (date, feature, stock) = (N, M, K) tensor
action: (action, confidence) = (K, 2) matrix
reward: (reward, K) matrix
"""
...
class agent(nn.Module):
    ...
    def __init__(self, environment,
        ...
        self.balance = 0
        self.profitloss = np.array([0] * agent.K) #종목별 수익률


    def make_action_list(self):
        for a in product([0, 1, 2], repeat=7):
            agent.ACTIONS.append(a)
        agent.ACTIONS = np.array(agent.ACTIONS)
        agent.NUM_ASSETS = agent.ACTIONS.shape[1]
    ...
    def get_action(self, state1, state2):
        with torch.no_grad():
            self.q_value = self.qnet(state1, state2)
        prob = np.random.uniform(low=0.0, high=1.0, size=1)
        if prob < self.epsilon:
            index = np.random.choice(agent.NUM_ACTIONS)
            action = agent.ACTIONS[index].reshape(-1, 1)
            confidence = np.array([0.5] * agent.K).reshape(-1, 1)
        else:
    ...
    def step(self, actions, confidences):
        assert actions.shape[0] == confidences.shape[0]

        close_p1 = self.environment.get_price()

        self.validate_action(actions)
        ...
        for i in range(actions.shape[0]):
            p1_price = close_p1[i]
            # Sell
            if actions[i] == agent.ACTION_SELL:
        ...
        return next_state1, next_state2, reward, done
